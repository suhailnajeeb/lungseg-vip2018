...
def main():
	...
	parser.add_option("--bs", "--batchSize", dest="batchSize", default=10)
	parser.add_option("--ed", "--extendDim", dest="extendDim", action='store_true', default=False)
	parser.add_option("--sw", "--swapTCh", dest="swapTimeChannel", action='store_true', default=False)
	parser.add_option("--mt", "--maskType", dest="maskType", default='2D')
	parser.add_option("--lr", "--learning_rate",dest="learningRate", default=0.001)
	parser.add_option("-p", "--patience", dest="patience", default=6)

	(options, args) = parser.parse_args()

	# database to use
	trainDbPath = 'P:/dataset3_3d_nSlice-9_cropped_x-74-426_y-74-426_clipped-0-1800_resized-224-224_aug-R90-R180-R270-Hf-Vf-Et_scaled_train.hdf5'
	valDbPath = '../dbHdf5/dataset3_3d_nSlice-9_cropped_x-74-426_y-74-426_clipped-0-1800_resized-224-224_scaled_val.hdf5'

	modelDir = '../models'              # where models are saved
	modelArch = 'LungNet3D002'            # model architecture to use from modelLib.py
	modelName = 'LungNet3D002_001'        # name to save model with
	...
	notes = "LungNet3D002 trained on 3d dataset of 9 slices (4 before and 4 after). Trained using LungNet2D weights to output 3d masks. 224x224 all augmentations on"
	...
	model.compile(loss=customLoss.log_dice_coef_3d_loss, loss_weights=[1], optimizer=adam, metrics=[customLoss.dice_coef_3d])

	trainGen = dataGenerator(trainDbPath,['slice'],['mask'],batchSize, extendDim=options.extendDim,numWorkers=numWorkers,swapTimeChannel=options.swapTimeChannel, maskType=options.maskType)
	testGen = dataGenerator(valDbPath,['slice'],['mask'],batchSize, extendDim=options.extendDim,numWorkers=numWorkers,swapTimeChannel=options.swapTimeChannel, maskType=options.maskType)
	...
	check5 = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=3, verbose=1, mode='auto', min_delta=0.001, cooldown=0, min_lr=1e-10)
